...
export const getProSessionCookie = async (): Promise<string | undefined> => {
  const cookieStore = await cookies();
  const sessionCookie = cookieStore.get("pro-auth")?.value;
...
export const isValidSessionCookie = async (
  ...
  // Redis deletes the database due to inactivity, so dont use it for now
